...
(async () => {
	...
	app.get('/cars', async (req: Request, res: Response) => {
		...
		if (make) {
			const carsByMake: Car[] = cars.filter(
				car => car.make === make.toString()
			);
			if (carsByMake.length !== 0) return res.status(200).send(carsByMake);
	...
	app.post('/cars', async (req: Request, res: Response) => {
		...
		const newCar: Car = { id, type, make, model, cost };
